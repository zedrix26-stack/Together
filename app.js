...
(function () {
    ...
    var IDB_VERSION = 2;
    var STORES = ['couples', 'photos', 'memories', 'moments', 'milestones', 'files'];
    ...
    function renderMore() {
        ...
        if (!el) return;
        var c = state.couple;
        var animOn = !!state.settings.animations;
        ...
        var shareHTML = '<div class="group"><h4 class="group-title">App</h4><div class="list">' +
            ...
            '<div class="list-item-main"><div class="list-item-label">Together</div><div class="list-item-sub">Version 1.0 · Your data stays on this device</div></div></div>' +
            '</div></div>';
        ...
        el.innerHTML =
            ...
            '</div></div></div></div>' +
            '<div class="group"><h4 class="group-title">Behavior</h4><div class="list">' +
            '<div class="list-item"><span class="ic">' + ic('sparkle', 20) + '</span>' +
            '<div class="list-item-main"><div class="list-item-label">Animations</div></div>' +
            '<label class="switch"><input type="checkbox" id="setAnimations"' + (animOn ? ' checked' : '') + '><span class="switch-track"></span></label></div>' +
            '</div></div>' +
            musicHTML +
    ...
    function showScreen(id) {
        ['screen-loading', 'screen-welcome', 'screen-main'].forEach(function (s) {
            var el = $('#' + s);
    ...
    function hideLoading() {
        ...
        requestAnimationFrame(function () {
            ...
            setTimeout(function () { ls.hidden = true; }, 380);
        });
    }
    ...
    function refreshCollections() {
        ...
        if (!pid) {
            state.photos = []; state.memories = []; state.moments = []; state.milestones = [];
        ...
        return Promise.all([
            ...
            state.photos = res[0]; state.memories = res[1]; state.moments = res[2]; state.milestones = res[3];
        });
    }
    ...
    function onMoreChange(e) {
        if (e.target.id === 'setMusic') {
            ...
            renderMore();
        } else if (e.target.id === 'setAnimations') {
