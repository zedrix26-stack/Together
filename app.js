...
(function () {
        ...
        importAll: function (data) {
            ...
            return Promise.all(jobs).then(function () {
                if (Array.isArray(data.chat) && data.chat.length) {
                    return Storage.replaceAll('chat', data.chat);
                }
            }).then(function () {
    ...
    function actionDelete() {
        ...
        confirmDialog({
            ...
            text: 'This will permanently remove it from both devices.',
            confirmText: 'Delete',
            icon: 'trash'
        }).then(function (ok) {
            if (!ok) return;
    ...
    var LiveCanvas = (function () {
        ...
        function loadFromDb() {
            ...
            Sync.client.from('live_canvas').select('*').eq('couple_id', Sync.coupleId).single().then(function (res) {
                ...
                try {
                    strokes = typeof res.data.strokes === 'string' ? JSON.parse(res.data.strokes) : (res.data.strokes || []);
                } catch (e) {
                    strokes = [];
                }
                redoStack = [];
