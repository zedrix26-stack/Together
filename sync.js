...
(function () {
    ...
    Sync.deleteAllData = function () {
        if (!this.ready || !this.authed || !this.coupleId) return Promise.resolve();
        var self = this;
        var coupleId = this.coupleId;
        var tables = ['chat', 'photos', 'memories', 'moments', 'milestones', 'live_canvas'];
        var deletes = tables.map(function (t) {
            return self.client.from(t).delete().eq('couple_id', coupleId).then(function (res) {
                if (res.error) console.warn('[Sync] deleteAllData', t, res.error);
            }).catch(function () {});
        });
        return Promise.all(deletes).then(function () {
            return self._removeCoupleFiles(coupleId);
        }).catch(function () {});
    };
